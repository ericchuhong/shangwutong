...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        [self initWithFrame:CGRectMake(0, 0-20, self.frame.size.width,self.frame.size.height-20 )];
    }
    return self;
}
- (void)dealloc
...
- (void)initSubViews
{
    ...
    for (int i = 0; i < allItemsCount; i++) {
        ...
        NSLog(@"backScrollView.frame.size.height*3/4:%lf",backScrollView.frame.size.height*3/4);
...
//        NSLog(@"grip's frame:%lf ",self.frame.size.height*1/4-self.frame.size.height*1/15);
...
//        [gripImgView release];
    ...
    NSLog(@"self.frame.size.height*3/4:%lf",self.frame.size.height*3/4);
    ...
    if (self.selectedImgView==nil) {
        ...
        NSLog(@"self.frame.size.height*1/3-self.frame.size.height*1/20:%lf",self.frame.size.height*1/3-self.frame.size.height*1/20);
...
- (id)initWithFrame:(CGRect)frame withDataSource:(id<BFSegmentControlDataSource>)mDataSource withSepratorLineImageName:(UIImage *)sepratorImage withBackgroundImageName:(UIImage *)backImage withLeftTagImage:(UIImage *)leftImage withRightTagImage:(UIImage *)rightImage
{
    if (self = [super initWithFrame:frame]) {
        NSLog(@"self.frame:%lf",self.frame.size.height);
...
- (void)selectedImgViewShouldMoveToIndex:(NSInteger)index
{
    ...
    NSLog(@"%lf",self.frame.size.height*1/3-self.frame.size.height*1/20);
...
- (void)setRightTagViewHidden:(BOOL)state
{
    ...
    NSLog(@"%lf",self.frame.size.width-self.frame.size.width*1/9);
